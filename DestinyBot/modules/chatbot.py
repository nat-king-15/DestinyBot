...
async def hmm(_, message):
    ...
    if status == "ON" or status == "on" or status == "On":
        ...
        if not lol:
            ...
            return
        await lel.edit(
            f"Destiny's AI Actived by {message.from_user.mention()} for users in {message.chat.title}"
        )
        ...
        if not Escobar:
            ...
            return
        await lel.edit(
            f"Destiny's AI Deactivated by {message.from_user.mention()} for users in {message.chat.title}"
        )
